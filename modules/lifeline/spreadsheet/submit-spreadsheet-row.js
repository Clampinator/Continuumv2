...
export async function submitSpreadsheetRow(actor, eventId, field, value) {
    ...
    if (locationFields.includes(field)) {
        ...
        const updateData = {
            ...
            // PULLED SPAN: Pass through from spreadsheet data
            isPulled: Boolean(rec.isPulled),
...
export async function submitNewRow(sheet, fv, options = {}) {
    ...
    const { eventIsSpan, isPulled, normalizedFacts } = classifyEventType(
        {
            eventIsSpan: Boolean(fv.eventIsSpan),
            isPulled: Boolean(fv.isPulled),
