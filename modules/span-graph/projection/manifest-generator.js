...
export function generateManifest(state, viewport, interaction = null) {
    ...
    state.segments.forEach((seg, index) => {
        ...
        if (seg.exitPoint) {
            ...
            if (nextSeg) {
                ...
                // PULLED SPAN: When another spanner carries this character,
                // the span rail renders with a distinct pattern.
                const isPulled = Boolean(seg.exitPoint.isPulled || seg.exitPoint.record?.isPulled);
    ...
    manifest.nodes = state.nodes.map(node => {
        ...
        const manifestNode = {
            ...
            isPreview,
            isPulled: Boolean(node.isPulled || node.record?.isPulled),
