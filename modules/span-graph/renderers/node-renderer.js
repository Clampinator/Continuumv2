...
export class NodeRenderer {
  ...
  _createNodeShape(node) {
    ...
    if (node.type === 'span-origin') {
        ...
        // PULLED SPAN: Outlined triangle (no fill) in soft magenta.
        // Self-spanned: Solid fill in standard magenta.
        if (node.isPulled) {
            shape.style.fill = 'transparent';
            shape.style.stroke = '#ffaaff';
        ...
        // PULLED SPAN: Softer magenta fill for destination semicircle
        if (node.isPulled) {
            shape.style.fill = '#ffaaff';
            shape.style.stroke = '#ffaaff';
        } else {
            shape.style.fill = '#ff00ff';
