...
export class RailRenderer {
  ...
  render(manifest) {
    ...
    manifest.rails.forEach(rail => {
        if (rail.type === 'level') {
            ...
            // PULLED SPAN: dash-dot pattern, softer color, thinner line.
            // The character was carried by another spanner - visually distinct.
            const el = this._createPathElement(pathData,
                rail.isPulled ? 'span-graph-span-line-pulled' : 'span-graph-span-line');
  ...
  _injectStyles() {
      ...
      style.textContent = `
          ...
          .span-graph-span-line-pulled {
              stroke: #ffaaff;
              stroke-width: 2;
              stroke-dasharray: 8, 3, 2, 3;
              opacity: 0.7;
              animation: span-flow 0.8s linear infinite;
          }
          .span-graph-yet-rail {
              stroke: #00e5ff;
              stroke-width: 1.5;
