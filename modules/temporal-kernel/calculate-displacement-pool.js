 ...
 *
 * PULLED SPANS: When a node's record has isPulled: true, another spanner
 * carried the character. That span's displacement does NOT count against
 * the character's own pool.
 *
 * @param {Array} physicalNodes - Array of physics nodes with { y, arrivalY, isSpanOrigin, isPulled, record }.
...
export function calculateDisplacementPool(physicalNodes) {
  ...
  for (let i = physicalNodes.length - 1; i >= 0; i--) {
    ...
    const isPulled = Boolean(record.isPulled || node.isPulled);
    // PULLED SPANS: Skip pool cost. The character was carried by another spanner.
