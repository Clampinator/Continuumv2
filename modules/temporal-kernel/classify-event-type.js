 ...
 *
 * PULLED SPANS: isPulled is only meaningful when eventIsSpan is true.
 * If the event is not a span, isPulled is forced to false.
 *
 * @param {Object} rawFacts - Raw form values.
 *   { eventIsSpan, isPulled, eventSpanFromDate, eventSpanToDate,
 ...
 * @returns {{ eventIsSpan: boolean, isPulled: boolean, hasSpanFacts: boolean, normalizedFacts: Object }}
 */
export function classifyEventType(rawFacts, constraints) {
  const {
    eventIsSpan: rawIsSpan,
    isPulled: rawIsPulled,
    eventSpanFromDate,
    eventSpanToDate,
    eventSpanFromTime,
    eventSpanToTime,
  ...
  // PULLED SPAN: Only meaningful when eventIsSpan is true.
  // A pulled span was caused by another spanner - zero pool cost.
  // Forces to false when not a span (same pattern as spanDisabled).
  const isPulled = eventIsSpan && Boolean(rawIsPulled);
  ...
  return { eventIsSpan, isPulled, hasSpanFacts, normalizedFacts };
}