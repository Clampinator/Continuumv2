...
function getLocationsInRange(lat, lng) {
    return (game.actors?.contents ?? []).filter(a => {
        ...
        if (!a.getFlag('continuum', 'revealedOnOrgMap')) return false;
...
function pickEngagement(engagements) {
    return new Promise((resolve) => {
        ...
        new Dialog({
            ...
            default: "ok"
        }, { classes: ["continuum", "dialog"] }).render(true);
    });
}
...
export async function handleOrgSituationClick(sheet, event) {
    ...
    const content = await renderTemplate("systems/continuum/templates/chat-org-attack.html", {
        unitName,
