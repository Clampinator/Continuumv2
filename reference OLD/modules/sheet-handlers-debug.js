...
export async function handleTimelineSortToggle(sheet, event) {
    event.preventDefault();
    const currentDir = sheet.actor.getFlag('continuum', 'timelineSortDirection') || 'desc';
    await sheet.actor.setFlag('continuum', 'timelineSortDirection', currentDir === 'desc' ? 'asc' : 'desc');
}
