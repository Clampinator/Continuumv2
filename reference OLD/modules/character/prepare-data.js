...
export async function prepareCharacterData(sheet, options) {
    const context = await ActorSheet.prototype.getData.call(sheet, options);
    ...
    context.toggles = sheet.actor.getFlag('continuum', 'sheetState.toggles') || {};
    context.timelineSortDirection = sheet.actor.getFlag('continuum', 'timelineSortDirection') || 'desc';
    context.playersCanSeeSpan = sheet.actor.getFlag('continuum', 'playersCanSeeSpan') ?? false;
    context.playersCanSeeMetabilities = sheet.actor.getFlag('continuum', 'playersCanSeeMetabilities') ?? false;
    context.playersCanSeeNaturalSpan = sheet.actor.getFlag('continuum', 'playersCanSeeNaturalSpan') ?? false;
    context.showErasSection = sheet.actor.getFlag('continuum', 'showErasSection') ?? false;
    context.showRelationshipsSection = sheet.actor.getFlag('continuum', 'showRelationshipsSection') ?? false;
    context.showGoalsSection = sheet.actor.getFlag('continuum', 'showGoalsSection') ?? false;
