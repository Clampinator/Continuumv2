...
export async function handleCharacterSettingsClick(sheet, event) {
    ...
    const showAgesSection = actor.getFlag('continuum', 'showAgesSection') ?? false;
    const showRelationshipsSection = actor.getFlag('continuum', 'showRelationshipsSection') ?? false;
    const showGoalsSection = actor.getFlag('continuum', 'showGoalsSection') ?? false;
    const spaceTimeLinked = actor.getFlag('continuum', 'spaceTimeLinked') ?? false;
    ...
    new Dialog({
      ...
      buttons: {
        save: {
          ...
          callback: async (html) => {
            const updates = {
                'flags.continuum.playersCanSeeSpan': html.find('input[name="playersCanSeeSpan"]').is(':checked'),
                'flags.continuum.playersCanSeeNaturalSpan': html.find('input[name="playersCanSeeNaturalSpan"]').is(':checked'),
                'flags.continuum.playersCanSeeMetabilities': html.find('input[name="playersCanSeeMetabilities"]').is(':checked'),
                'flags.continuum.showAgesSection': html.find('input[name="showAgesSection"]').is(':checked'),
                'flags.continuum.showRelationshipsSection': html.find('input[name="showRelationshipsSection"]').is(':checked'),
                'flags.continuum.showGoalsSection': html.find('input[name="showGoalsSection"]').is(':checked'),
                'flags.continuum.spaceTimeLinked': html.find('input[name="spaceTimeLinked"]').is(':checked'),
