...
export class NPCWizardApp extends Application {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      ...
      title: 'NPC Generator',
      template: 'systems/continuum/templates/npc-generator/npc-wizard.html',
      classes: ['continuum', 'npc-wizard'],
      width: 700,
      height: 680,
      resizable: true
    });
  ...
  async _render(force, options) {
    await getTemplate('systems/continuum/templates/npc-generator/step-6-complete.html');
...
html.find('#npc-name').on('input', (e) => { this.state.data.name = e.target.value; });
  ...
  async _handleGenerate(html) {
    ...
    try {
      const apiKey = game.settings.get('continuum', 'geminiApiKey');
      if (!apiKey) {
        addLog('ERROR: AI API key not configured. Go to Settings > Configure Settings > System Settings.');
        return;
      }

      const provider = game.settings.get('continuum', 'aiProvider') || 'gemini';
      ...
      const stabilityKey = game.settings.get('continuum', 'stabilityAiKey');
      if (stabilityKey) {
