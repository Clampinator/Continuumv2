...
function _resolveLocationCoords(locationName) {
    ...
    const lat = locationActor.system?.map?.lat ?? locationActor.getFlag('continuum', 'latitude') ?? null;
    const lng = locationActor.system?.map?.lng ?? locationActor.getFlag('continuum', 'longitude') ?? null;
    const zoom = locationActor.getFlag('continuum', 'zoom') ?? null;
    return { lat, lng, zoom };
}
