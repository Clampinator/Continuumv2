...
- **Pulled Span**: `isPulled: true` on a span event means another spanner carried the character. The span exists on the timeline but costs zero from the character's own pool. `classifyEventType` forces `isPulled` to false when `eventIsSpan` is false. `calculate-span-pool` and `calculate-displacement-pool` skip pulled spans in their cost accumulation.
- The **only** thing allowed to tell the DB how to change
- Files: `apply-collision-laws.js`, `calculate-span-displacement.js`, `calculate-span-pool.js`, `calculate-displacement-pool.js`, `classify-event-type.js`, `establish-history-physics.js`, `project-diagonal.js`, `project-subjective-age.js`, `resolve-narrative-order.js`, `solve-history-physics.js`, `solve-now-drag-constraint.js`, `validate-span-physics.js`
...
In the Continuum RPG, a higher-rank spanner can **pull** a lower-rank character through time. The passenger travels with the puller but does **not** spend their own span pool. This is modeled by the `isPulled` boolean on span events.

**Data Flow:**
1. **Dialog** (`get-template-data.js`, `handle-submit.js`): `isPulled` checkbox visible when `eventIsSpan` is checked. Passed as form data.
2. **Kernel** (`classify-event-type.js`): Forces `isPulled` to `false` when `eventIsSpan` is false (non-span events cannot be pulled).
3. **Kernel** (`calculate-span-pool.js`, `calculate-displacement-pool.js`): Skips pulled spans in pool/displacement accumulation. Pulled spans still show their displacement for display (formatted as `"Xy (PULLED)"`).
4. **State** (`insert-history-row.js`, `update-history-row.js`, `get-actor-history.js`): Persists and passes through `isPulled`.
5. **Engine** (`establish-history-physics.js`): Includes `isPulled` in physical nodes.
6. **Projector** (`manifest-generator.js`): Passes `isPulled` to manifest entries for both rails and nodes.
7. **Renderers** (`rail-renderer.js`, `node-renderer.js`): Pulled spans render as dash-dot lines in `#ffaaff` with 0.7 opacity. Origin nodes are outlined triangles (no fill). Destination nodes are filled semicircles in `#ffaaff`.

**Rules:**
- Level Breath still applies to pulled spans. The character's body experienced consecutive time jumps.
- A pulled span still occupies the timeline position (subjective age, objective time). Only the pool cost is zeroed.
