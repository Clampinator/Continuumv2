...
## Validation Checklist (Definition of Done) [checkpoint: f579294]
- [x] **Unit Tests Pass:** `npm test -- tests/temporal-translator/age-converter.test.js` returns 100% passing tests and >80% coverage.
- [x] **Parsing Accuracy:** Verified that `"1y"` parses to `31536000` seconds exactly.
- [x] **Formatting Accuracy:** Verified that `31622400` seconds formats back to `"1y 1d 00:00:00"` (or equivalent standard).
- [x] **Resilience:** Garbage strings like `"abc"` or `NaN` parse to `0` safely.
- [x] **No Context Leak:** The module contains no references to `actor`, `document`, or global state.
