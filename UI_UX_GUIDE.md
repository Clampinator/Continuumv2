...
*   **Pulled Span Jumps**: Soft Magenta (`#ffaaff`), 2px dash-dot pattern (`8,3,2,3`), 0.7 opacity. Another spanner carried the character; the rail must be visually distinct from self-spanned rails.
*   **Pulled Span Origin Node**: Outlined triangle (no fill), `#ffaaff` stroke, `4,2` dash, 2px width.
*   **Pulled Span Dest Node**: Filled semicircle in `#ffaaff`, `#ffaaff` stroke.
*   **NOW Cursor**: High-Visibility Yellow/Gold (`#ffd700`).
*   **Birth Node**: Solid Gold circle.
*   **Tooltips**: Pure White labels (`#ffffff`) with Neon Cyan data values (`#00ffff`). Pulled spans show displacement with "(PULLED)" annotation.
